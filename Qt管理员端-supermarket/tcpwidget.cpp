...
void TcpWidget::receiveMessages()
{
    ...
    */
    ...
    }else if(str_list[0] == "会员列表")
    {
        emit recv_menlist_singal(str_list[1]);
...
void TcpWidget::send_request_memlist()
{
    QString str = "请求会员列表";
    textBrowser->append("客户端：" + str);
    tcpSocket->write(str.toUtf8().data());
...
void TcpWidget::send_request_save_memlist(QString memlist)
{
    QString str = "请求保存会员列表:"+memlist;
    textBrowser->append("客户端：" + str);
    tcpSocket->write(str.toUtf8().data());
}
void TcpWidget::send_request_save_goodslist(QString goodslist)
{
    QString str = "请求保存商品列表:"+goodslist;
    textBrowser->append("客户端：" + str);
    tcpSocket->write(str.toUtf8().data());
}
